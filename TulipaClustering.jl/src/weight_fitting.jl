...
function fit_rep_period_weights!(
    ...
    allowed_args = (:niters, :learning_rate, :adaptive_grad)
    filtered_args = (; (k => args[k] for k in allowed_args if haskey(args, k))...)

    alpha = get(args, :gamma, 0.0)
    beta = get(args, :beta, 0.0)
    ...
    for period in periods
        ...
        if alpha > 1e-6 && beta > 1e-6
            error(
                "You can either penalize by distance or only artificial periods, not together",
            )
        elseif beta > 1e-6
            #with distance
            d = sum((rp_matrix .- target_vector) .^ 2; dims = 1)[:]
            subgradient = (x) -> rp_matrix' * (rp_matrix * x - target_vector) + beta * d
        elseif alpha > 1e-6
            # gamma based on R (only penalize artificial)
            mask = zeros(n_rp)
