...
function fit_rep_period_weights!(
    ...
    for period in periods
        ...
        distances = [norm(target_vector - rp_matrix[:, j]) for j in 1:n_rp] # MODIFY THIS IN CASE TO USE OTHER DISTANCES
        idx = partialsortperm(distances, 1:k)
        rp_sub = rp_matrix[:, idx]
        if weight_type == :conical_bounded
            n_data_points = size(rp_matrix, 1)
            rp_sub = hcat(rp_sub, repeat([0.0], n_data_points))
        end
        subgradient = (x) -> rp_sub' * (rp_sub * x - target_vector)

        if weight_type == :conical_bounded
            x = vcat(Vector(weight_matrix[period, idx]), [0.0])
        else
            x = Vector(weight_matrix[period, idx])
        end
        ...
        full_x[idx] = x
        if is_sparse
            full_x = sparse(full_x)
        end
        weight_matrix[period, 1:n_rp] = full_x
    end
