...
function find_representative_periods(
    ...
    # now implement iteratively: m iterations and everytime we add a specific number of extreme periods that are stored in the vector n_rp_second_divided

    m = 4 # number of iterations

    if n_rp_second <= m
        m = n_rp_second
        n_rp_second_divided = fill(1, m) # 1 for everyone
    else
        rp_each = div(n_rp_second, m)
        rest = n_rp_second - rp_each * m
        n_rp_second_divided = fill(rp_each, m)
        n_rp_second_divided[1] = n_rp_second_divided[1] + rest
    end
    ...
    add_with_kmedoids = 0 # if we have finished periods that are represented by the artificial ones we add these with k-medoids

    # now we do it iteratively m times
    for i in 1:m

        # 4.3 Let's compute the weights to choose which rps to add on top of the others
        ...
        n_rp_second_eff_div = min(n_rp_second_divided[i], length(positive_rows))
        top_rows = positive_rows[partialsortperm(
            row_sums[positive_rows],
            1:n_rp_second_eff_div;
            rev = true,
        )]
        ...
        hull_indices = union(hull_indices, top_rows)

        weight_matrix =
            spzeros(n_complete_periods, size(weight_matrix, 2) + n_rp_second_eff_div)  # size: what was there before adding what is new

        rp_matrix = _combine_matrices([rp_matrix, clustering_matrix[:, top_rows]])
        println("after  $i ", size(rp_matrix))
        ...
        if size(rp_matrix, 2) !=
           n_rp_first + sum(n_rp_second_divided[1:(i - 1)]) + n_rp_second_eff_div
            error(
        ...
        if n_rp_second_divided[i] != n_rp_second_eff_div # in this case we stop and continue with k-medoids
            add_with_kmedoids =
                n_rp_second_divided[i] - n_rp_second_eff_div +
                sum(n_rp_second_divided[(i + 1):m])
            n_rp_sd = n_rp_second_divided[i]
            @warn "Wanted to add more extreme periods than necessary at iteration $i : $n_rp_second_eff_div vs $n_rp_sd"
            println("Continue with kmedoids for the last $add_with_kmedoids periods")
            break
        end
    end

    if add_with_kmedoids > 0
        # 4.4 Now let's cluster more to gain optimality if we have representative periods available
        clustering_matrix_remaining =
            clustering_matrix[:, setdiff(axes(clustering_matrix, 2), hull_indices)] # remove columns with period already represented
        distance_matrix = pairwise(distance, clustering_matrix_remaining; dims = 2)
        kmedoids_result = kmedoids(distance_matrix, add_with_kmedoids)
    ...
    println("after all ", size(rp_matrix))
