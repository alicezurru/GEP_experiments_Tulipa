function add_objective!(connection, model, variables, expressions, profiles, model_parameters)
    ...
    penalty_loss_of_load = JuMP.AffExpr(0.0)
    for row in indices
        coefficient = 0.0
        # added by me
        total_cost =
            ismissing(row.total_cost_if_no_profile) ? 0.0 : row.total_cost_if_no_profile::Float64
        cost_coef = ismissing(row.cost_coefficient) ? 0.0 : row.cost_coefficient::Float64
        ...
        # added by me
        if row.from_asset == "ens" && row.to_asset == "e_demand"
            JuMP.add_to_expression!(penalty_loss_of_load, coefficient, var_flow[row.id::Int64])
        end
    end

    model[:flows_operational_cost] = flows_operational_cost
    model[:penalty_loss_of_load] = penalty_loss_of_load
