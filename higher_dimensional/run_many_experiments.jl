        ...
        for rp in representative_periods
            ...
            for n in 1:n_runs
                ...
                if add_worst_every_hour
                    ...
                    create_init_rps_hourly(connection, profiles_type, period_duration, profiles) # tested only for 1 year
                ...
                if add_worst_sum
                    ...
                    create_init_rps_daily(connection, profiles_type, period_duration, profiles) # tested only for 1 year
                end
                ...
                if add_worst_sum_real
                    ...
                    create_init_rps_daily_real(connection, profiles_type, period_duration, profiles) # tested only for 1 year
                end

                # to use the ratio availability/demand
                if use_ratio == true # this works now that we have only one demand location and one availability, so we divide availability by that only demand
                    DuckDB.query(
                        connection,
                        "
