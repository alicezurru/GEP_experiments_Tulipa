...
n_experiments = config["simulation"]["n_experiments"]
n_scenarios = config["simulation"]["n_scenarios"]
...
representative_periods = [20, 40, 60, 80, 100, 150, 300, 450]
enable_names = true
direct_model = false
...
function run_experiments_fixedseed(n_exp::Int; rng=Random.GLOBAL_RNG)

    @info "Running experiment batch $n_exp"
    ...
    # select randomly n_scenarios
    input_df = TIO.get_table(connection_benchmark, "profiles_wide_$profiles_type")
    scenarios = unique(input_df.scenario)
    selected = sort(shuffle(rng, scenarios)[1:n_scenarios])
    profiles_selected_wide_df = filter(row -> row.scenario in selected, input_df)
    profiles_selected_wide_df |> CSV.write("outputs/profiles_check.csv"; writeheader=true)
    DuckDB.register_data_frame(connection_benchmark, profiles_selected_wide_df, "tmp")

    DBInterface.execute(
        connection_benchmark,
        """
    CREATE OR REPLACE TABLE profiles_selected_wide AS
    SELECT * FROM tmp
"""
    )
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            for n in 1:n_runs
                ...
                # transform the profiles data from wide to long
                DuckDB.register_data_frame(connection, profiles_selected_wide_df, "tmp")
...
for i in 1:n_experiments
    ...
    run_experiments_fixedseed(i; rng=rng)
end
results_df |> CSV.write("outputs/results.csv"; writeheader=true)

# df = CSV.read("outputs/results.csv", DataFrame)
# df.n_experiment .= df.n_experiment .+ 5
# df |> CSV.write("outputs/results2.csv"; writeheader=true)
