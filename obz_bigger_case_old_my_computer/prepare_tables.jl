...
TulipaClustering.transform_wide_to_long!(connection, "profiles_wide", "pivot_profiles"; exclude_columns=["year", "timestep"])
DuckDB.query(
    connection,
...
DuckDB.query(
    ...
    SELECT
        ...
        CASE 
            WHEN LOWER(tay.name) LIKE '%battery%' THEN 8 * a.capacity 
            ELSE 0.0
...
DuckDB.query(
    ...
    SELECT
        ...
        year AS milestone_year,
        CASE
            WHEN ends_with(from_asset, '_ENS') THEN 50000
            WHEN LOWER(from_asset) LIKE '%smr%' THEN 50000
        ELSE variable_cost
