...
# Rows (now scenarios instead of artificial cases)
ROW_SCENARIOS = ["DISTANT", "CLOSE"]
ROW_LABELS = ["DISTANT", "CLOSE"]

# Columns (beta values)
BETAS = ["b0", "b1", "g1"]
BETA_LABELS = [r"$\beta,\gamma=0$", r"$\beta=1$", r"$\gamma=1$"]
...
def plot_values_grid(results_sets, value, savepath):
    ...
    for row_idx, scenario in enumerate(ROW_SCENARIOS):
        for col_idx, beta in enumerate(BETAS):
            ax = axes[row_idx, col_idx]
            df = results_sets[row_idx][beta]
...
def main():
    ...
    for scenario in ["dist", "cl"]:
        beta_dict = {}

        for beta in BETAS:
            beta_str = str(beta)
            df = pd.read_csv(
                SCRIPT_DIR / f"results_{scenario}_{beta_str}.csv"
            )
