...
ROW_KEYS = ["", "h", "d"]  # suffixes for CSV names
...
def plot_values_grid(results_sets, value, savepath):
    ...
    rp_vals = sorted(v for v in results_sets[0][COLS[0]].rp.unique() if v != 1)
    ...
    for row_idx, results_dict in enumerate(results_sets):
        for col_idx, col in enumerate(COLS):
            ...
            if col_idx == 0:
                ax.set_ylabel(ROW_LABELS[row_idx], fontsize=20, labelpad=37)
    ...
    fig.supxlabel("Number of representative periods", fontsize=17)
...
def main():
    ...
    for suffix in ROW_KEYS:
        results = {
            "DISTANT": pd.read_csv(SCRIPT_DIR / f"results_dist{suffix}.csv"),
            "HALFMIXED": pd.read_csv(SCRIPT_DIR / f"results_hmix{suffix}.csv"),
            "CLOSE": pd.read_csv(SCRIPT_DIR / f"results_cl{suffix}.csv"),
            "MIXED": pd.read_csv(SCRIPT_DIR / f"results_mix{suffix}.csv"),
        }
