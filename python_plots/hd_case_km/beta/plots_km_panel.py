...
# We now want CLOSE and DISTANT as rows
ROWS = [
    ("DISTANT", "DISTANT"),
    ("CLOSE", "CLOSE"),
]
...
def plot_values_quantiles_grid(stats_dict, case_df, value, savepath):
    ...
    rp_vals = sorted(
        v for v in stats_dict["DISTANT"]["b0"]["rp"].unique() if v != 1
    )
    ...
    for row_idx, (row_title, key) in enumerate(ROWS):
        for col_idx, beta in enumerate(BETAS):
            ...
            stats_df = stats_dict[key][beta]

            plot_panel(ax, stats_df, case_df, value, rp_to_pos)

            if row_idx == 0:
                ax.set_title(r"$\beta$ = " + beta[1:], fontsize=16)
...
def main():
    ...
    stats = {}
    for key in ["DISTANT", "CLOSE"]:
        base = FILE_PREFIX[key]
        stats[key] = {}
        for beta in BETAS:
            stats[key][beta] = pd.read_csv(
                SCRIPT_DIR / f"{base}_{beta}.csv"
            )
