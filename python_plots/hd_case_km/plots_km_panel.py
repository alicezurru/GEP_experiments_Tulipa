...
ROWS = [
    ("NO ARTIFICIAL", ""),
    ("HCWC DIRAC WEIGHTS", "_dirac"),
    ("HCWC CONVEX WEIGHTS", "_convex"),
]
...
def plot_values_quantiles_grid(stats_dict, case_df, value, savepath):
    rp_vals = sorted(
        v for v in stats_dict[COLS[0]][""]["rp"].unique() if v != 1
    )
...
def main():
    ...
    for col in COLS:
        ...
        stats[col] = {
            "": pd.read_csv(SCRIPT_DIR / f"{base}.csv"),
            "_dirac": pd.read_csv(SCRIPT_DIR / f"{base}_dirac.csv"),
            "_convex": pd.read_csv(SCRIPT_DIR / f"{base}_convex.csv"),
        }
