...
def load_all():
    ...
    for case, path in FILES.items():
        ...
        df["method"] = df["base_name"].str.split("_").str[0]
        df["type"]   = df["base_name"].str.split("_").str[1]
...
def main():
    ...
    plot_grid(
        df,
        method="convex",
        savepath=PLOTS_DIR / "lol_convex_boxplot.png"
    )

    plot_grid(
        df,
        method="conicalb",
        savepath=PLOTS_DIR / "lol_conicalb_boxplot.png"
    )
